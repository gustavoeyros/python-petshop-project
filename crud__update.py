...
def update():
    ...
    if (id == "" or name == "" or phone == ""):
        ...
        cursor.execute("UPDATE usuario SET nome='"+name +
                       " ', telefone='"+phone + "' WHERE codigo='"+id+"'")
        cursor.execute("commit")

        MessageBox.showinfo("Success", "Dados atualizados com sucesso")
        connection.close()
...
btnSave = Button(root, text="Atualizar",
                 font="Inter 10 bold", fg="white", bg="#8C30F5", border=5, background="#8C30F5", bd=0, width=20, height=2, command=update)
